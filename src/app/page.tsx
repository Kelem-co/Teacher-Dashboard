...
const MetricCard = ({ 
  ...
  <div className="bg-white border border-slate-100 p-6 rounded-2xl flex items-center justify-between shadow-sm">
    <div>
      ...
      <h3 className="text-3xl font-mono font-bold text-slate-900 tabular-nums">{value}</h3>
...
const MessageRow: React.FC<MessageRowProps> = ({ 
  ...
  <div className="py-4 border-b border-slate-50 last:border-0 group cursor-pointer" onClick={onReply}>
    <div className="flex justify-between items-start mb-1">
      <span className="text-xs font-bold text-slate-900 uppercase tracking-tight">{sender}</span>
      <span className="text-[10px] font-mono text-slate-400 uppercase">{time}</span>
    </div>
...
const TimelineItem = ({ 
  ...
  <div className="flex items-center gap-6 py-4 border-b border-slate-50 relative group">
    ...
    <div className="flex-1 flex items-center gap-3">
      <h4 className="text-sm font-bold text-slate-800 tracking-tight">{subject}</h4>
      {isLive && (
...
export default function App() {
  ...
  const [attendanceView, setAttendanceView] = useState<'Day' | 'Week' | 'Month'>('Day');
  ...
  const getDayStats = (date: Date) => {
    ...
    const present = Object.values(dayData).filter(v => v === 'present').length;
  ...
  const getStudentLifecycleStats = (studentId: string) => {
    ...
    const rate = totalClasses > 0 ? ((present + late) / totalClasses) * 100 : 100;
      ...
      if (studentId) {
        ...
        if (student) {
          const existingThread = messageThreads.find(t => 
            ...
            t.studentName.toLowerCase().includes(student.name.split(' ')[0].toLowerCase()) ||
            t.parentName.toLowerCase().includes(student.parentName.split(' ')[0].toLowerCase())
          );
          
          if (existingThread) {
            setActiveMessageThreadId(existingThread.id);
          } else {
            const pInitials = student.parentName.split(' ').map(n => n[0]).join('') || 'P';
            const newThreadId = `THR-${Date.now()}`;
            const newThread: Thread = {
              id: newThreadId,
              parentName: student.parentName,
              parentInitials: pInitials,
              parentPhone: student.parentPhone ?? ("+251 9" + Math.floor(10000000 + Math.random() * 90000000)),
              parentEmail: student.parentEmail ?? `${student.parentName.toLowerCase().replace(/[^a-z]/g, '')}@mail.com`,
              studentName: student.name,
              studentId: student.id,
              studentGrade: student.section ?? "Grade 7A",
              avatarColor: ["blue", "teal", "purple", "amber", "green"][Math.floor(Math.random() * 5)],
              unread: false,
              lastTime: "Just now",
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900 selection:bg-blue-100 selection:text-[#1A237E]">
      ...
      {/* A. Left Sidebar */}
      <aside className={`w-64 bg-white border-r border-slate-100 flex flex-col fixed inset-y-0 z-50 transition-transform duration-300 transform lg:translate-x-0 ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Header Context */}
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2 px-2.5 py-1 bg-[#1A237E]/8 rounded-full border border-[#1A237E]/10">
                <Shield size={10} className="text-[#1A237E]" fill="currentColor" />
                <span className="text-[9px] font-black text-[#1A237E] uppercase tracking-[0.2em] leading-none">Kelem-Co Platform</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-linear-to-br from-[#1A237E] to-[#3949AB] rounded-xl flex items-center justify-center shadow-md shadow-blue-900/15 shrink-0 ring-1 ring-white/20">
                <Hexagon className="text-white fill-white/10" size={18} strokeWidth={2.5} />
              </div>
              <div className="flex flex-col min-w-0">
                <h2 className="text-[12px] font-black uppercase tracking-tight text-slate-800 truncate leading-tight">
                  EDUGOV School
                </h2>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Academic Portal</p>
              </div>
            </div>
          </div>
        ...
        <nav className="flex-1 px-4 py-2 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <SidebarItem icon={Home} label="Homeworks" isActive={activeTab === 'Homeworks'} onClick={() => setActiveTab('Homeworks')} />
          <SidebarItem 
            icon={MessageSquare} 
            label="Messages" 
          ...
          />
          <SidebarItem icon={Bell} label="Notifications" isActive={activeTab === 'Notifications'} count={notificationCount} onClick={() => setActiveTab('Notifications')} />
        </nav>
      ...
      <main className="lg:ml-64 flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden h-16 bg-white border-b border-slate-100 px-6 flex items-center justify-between sticky top-0 z-40">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-slate-900 uppercase tracking-tight">Ethio-Global</span>
          </div>
          <button 
          ...
          >
            <Bell size={20} />
            {notificationCount > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center border-2 border-white">{notificationCount}</span>
            )}
          </button>
        </header>

        {/* Dashboard Grid Container */}
        <div className={`${activeTab === 'Messages' ? 'p-0 gap-0 overflow-hidden' : 'p-4 md:p-8 gap-8 overflow-y-auto'} flex flex-col flex-1 w-full max-w-full`}>
          {activeTab === 'Overview' && (
                  ...
                  <div className="lg:sticky lg:top-8 space-y-6 lg:mt-0">
                    ...
                    <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                      <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                        <h3 className="text-sm font-bold text-slate-800 tracking-tight uppercase">Parent Messages</h3>
                        <button 
                          onClick={() => setActiveTab('Messages')}
                      ...
                      <div className="px-6 py-2">
                        {messageThreads.filter(t => t.unread).length > 0 ? (
                          messageThreads.filter(t => t.unread).slice(0, 4).map(thread => (
                            <MessageRow 
                              key={thread.id}
                              ...
                              onReply={() => {
                                ...
                                setMessageThreads(prev => prev.map(t => t.id === thread.id ? { ...t, unread: false } : t));
                              }}
                            />
                          ))
                        ) : (
          ...
          {activeTab === 'Attendance' && (
            <div className="flex-1 space-y-6">
              ...
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm">
                    ...
                    <div className="relative group/month">
                      ...
                      <div className="absolute top-full left-0 mt-2 w-64 bg-white border border-slate-100 shadow-2xl rounded-2xl p-4 invisible group-hover/month:visible opacity-0 group-hover/month:opacity-100 transition-all z-50">
                            ...
                            return (
                              <button
                                ...
                                className={`text-[9px] font-black py-2 rounded-lg transition-colors ${
                                  isCurrent ? 'bg-[#1A237E] text-white' : 'hover:bg-slate-50 text-slate-500'
                                }`}
                              >
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-50 flex justify-between items-center px-1">
                           <button onClick={() => setViewDate(new Date(viewDate.getFullYear() - 1, viewDate.getMonth(), 1))} className="text-[10px] font-bold text-slate-400 hover:text-slate-600 tracking-tighter">PREV YEAR</button>
                           <span className="text-[10px] font-black text-slate-800">{viewDate.getFullYear()}</span>
                           <button onClick={() => setViewDate(new Date(viewDate.getFullYear() + 1, viewDate.getMonth(), 1))} className="text-[10px] font-bold text-slate-400 hover:text-slate-600 tracking-tighter">NEXT YEAR</button>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-4 custom-scrollbar snap-x">
                    {daysInView.map((date, idx) => {
                      ...
                      return (
                        ...
                        >
                          <span className={`text-[9px] font-black uppercase mb-1 tracking-tight ${isSelected ? 'text-blue-100' : isWeekend ? 'text-slate-200' : 'text-slate-400'}`}>
                            {date.toLocaleDateString('en-US', { weekday: 'short' })}
                          </span>
                          <span className="text-xl font-mono font-black leading-tight">{date.getDate()}</span>
                          {isToday && !isSelected && (
                ...
                {attendanceView !== 'Day' && (
                  <div className={`p-4 rounded-2xl flex items-center justify-between ${attendanceView === 'Week' ? 'bg-slate-50/50' : 'bg-[#1A237E]/5'}`}>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${attendanceView === 'Week' ? 'text-slate-400' : 'text-[#1A237E]'}`}>
                      {attendanceView}ly Analytics Context: {formatMonth(viewDate)}
                    </span>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-emerald-500" />
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Active Analysis</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                <div className="p-4 md:p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <div className="flex flex-wrap items-center gap-4 md:gap-6">
                    ...
                    <div className="flex items-center gap-6 flex-1 md:flex-none justify-between md:justify-end">
                      ...
                      >
                        <p className={`text-[10px] font-black uppercase tracking-widest ${attendanceFilter === 'present' ? 'text-emerald-600' : 'text-slate-400'}`}>
                          {attendanceView === 'Day' ? 'Present' : 'Avg. Score'}
                        </p>
                        <p className={`text-xl font-mono font-bold ${attendanceView === 'Day' ? 'text-emerald-600' : 'text-[#1A237E]'}`}>
                          {attendanceView === 'Day' ? `${stats.present}/${stats.total}` : '94.2%'}
                        </p>
                      </button>
                      ...
                      {attendanceView === 'Day' && (
                        <>
                          ...
                          >
                            <p className={`text-[10px] font-black uppercase tracking-widest ${attendanceFilter === 'absent' ? 'text-red-500' : 'text-slate-400'}`}>Absent</p>
                            <p className="text-xl font-mono font-bold text-red-500">{stats.absent < 10 ? `0${stats.absent}` : stats.absent}</p>
                          </button>

                          <div className="w-px h-10 bg-slate-100" />
                          <button 
                            onClick={() => setAttendanceFilter(prev => prev === 'late' ? 'all' : 'late')}
                            className={`text-right group transition-all ${attendanceFilter === 'late' ? 'scale-110' : 'opacity-60 hover:opacity-100'}`}
                          >
                            <p className={`text-[10px] font-black uppercase tracking-widest ${attendanceFilter === 'late' ? 'text-amber-500' : 'text-slate-400'}`}>Late</p>
                            <p className="text-xl font-mono font-bold text-amber-500">{stats.late < 10 ? `0${stats.late}` : stats.late}</p>
                          </button>
                        </>
                      )}
                    </div>
                ...
                <div className="p-2 space-y-1">
                  {students.filter(s => {
                    ...
                    return attendance[selectedDate.toDateString()]?.[s.id] === attendanceFilter;
                  }).map((student, i) => {
                    const status = attendance[selectedDate.toDateString()]?.[student.id];
                        ...
                        <div className="flex items-center gap-4">
                          <div className="relative">
                            ...
                            {status && (
                              <div className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white shadow-sm ${
                                ...
                                status === 'absent' ? 'bg-red-500' : 'bg-amber-500'
                              }`} />
                            )}
                          </div>
                          <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="hidden sm:inline-block text-[9px] font-black text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest bg-white px-2 py-0.5 rounded-full border border-slate-100">View Profile</span>
                            </div>
                            <span className="text-[9px] font-mono text-slate-400 tabular-nums uppercase tracking-tighter">
                              {attendanceView === 'Day' ? `ID: EGA-7A-0${i + 1}` : 'Academic Profile Active'}
                            </span>
                          </div>
                        </div>

                        {attendanceView === 'Day' && (
                          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar" onClick={(e) => e.stopPropagation()}>
                            <button 
                        ...
                        <div className="flex items-center gap-2">
                          {[0, 1, 2, 3, 4].map((d) => (
                            <div key={d} className="flex flex-col items-center">
                              <span className="text-[7px] font-bold text-slate-300 mb-1 leading-none">{'MTWTF'[d]}</span>
                              <div className={`w-6 h-6 rounded-lg ${d % 3 === 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-300'} flex items-center justify-center border border-white shadow-sm ring-1 ring-slate-100`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${d % 3 === 0 ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {attendanceView === 'Month' && (
                        <div className="flex items-center gap-6">
                           <div className="text-right">
                              <span className="text-xs font-mono font-bold text-slate-700">{90 + (i % 10)}%</span>
                           </div>
                           <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${90 + (i % 10)}%` }} />
                           </div>
                ...
                <div className="p-8 bg-slate-50/50 border-t border-slate-50">
                  <button className="w-full py-4 bg-[#1A237E] text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-lg shadow-blue-900/20 hover:bg-blue-900 transition-all">
                    {attendanceView === 'Day' ? 'Submit Attendance Report' : `Export ${attendanceView}ly Summary`}
                  </button>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'Notifications' && (
            <div className="flex-1 max-w-4xl mx-auto space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <div>
                  <h3 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight">Notifications</h3>
                  <p className="text-[10px] md:text-xs font-medium text-slate-400 mt-1 uppercase tracking-widest">Stay updated with critical school alerts</p>
                </div>
                <button 
              ...
              <div className="space-y-4">
                {notifications.map((notif) => (
                  ...
                  >
                    {!notif.read && <div className="absolute top-0 left-0 w-1 h-full bg-[#1A237E]" />}
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                        <span className={`w-2 h-2 rounded-full ${notif.read ? 'bg-slate-200' : (notif.urgent ? 'bg-red-500' : 'bg-blue-500')}`} />
                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-2 py-1 rounded-lg ${notif.read ? 'bg-slate-50 text-slate-400' : (notif.urgent ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600')}`}>
                          {notif.urgent ? 'Critical Priority' : 'Information'}
                        </span>
                      </div>
                      <span className="text-[10px] font-mono text-slate-300 uppercase">{notif.time}</span>
                    </div>
                    <h4 className={`text-lg font-bold mb-2 ${notif.read ? 'text-slate-500' : 'text-slate-800'}`}>{notif.title}</h4>
                    <p className={`text-sm leading-relaxed max-w-2xl ${notif.read ? 'text-slate-400' : 'text-slate-500'}`}>{notif.desc}</p>
                    <div className="mt-6 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="text-[10px] font-black text-[#1A237E] uppercase tracking-widest hover:underline">Take Action</button>
          ...
          {!['Overview', 'Attendance', 'Notifications', 'Students', 'Tasks', 'Gradebook', 'Analytics', 'Homeworks', 'Messages', 'Schedule'].includes(activeTab) && (
            <div className="flex-1 flex flex-col items-center justify-center bg-white border border-slate-100 rounded-3xl p-12 text-center shadow-sm">
                <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center text-slate-300 mb-6 font-mono text-4xl">?</div>
                <h3 className="text-xl font-black text-slate-900 uppercase mb-2">{activeTab} View</h3>
                <p className="text-sm text-slate-400 max-w-xs font-medium">
                  We're currently finalizing the module for <span className="text-indigo-600 font-bold">{activeTab}</span>. Check back soon for the full feature set.
                </p>
                <button 
          ...
          <>
            ...
            >
                    ...
                    return (
                      <>
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[9px] font-black text-slate-400 tracking-[0.1em] uppercase mt-0.5">EGA ID: {student.id}</p>
                        </div>
                      </>
                    );
                  })()}
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar">
                  ...
                  return (
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100">
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Attendance Rate</p>
                        <p className="text-lg font-mono font-bold text-[#1A237E]">{sStats.rate.toFixed(1)}%</p>
                      </div>
                      <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100">
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Total Lates</p>
                        <p className="text-lg font-mono font-bold text-amber-500">{sStats.late < 10 ? `0${sStats.late}` : sStats.late}</p>
                      </div>
                    </div>
                  );
                })()}
                ...
                {/* Monthly Trend Graph */}
                <div>
                   <h4 className="text-[9px] font-black tracking-[0.15em] text-slate-400 uppercase mb-2">30-Day Density Map</h4>
                   <div className="h-20 w-full bg-slate-50 rounded-xl relative flex items-end justify-between p-3 px-4 overflow-hidden group">
                      {Array.from({ length: 30 }, (_, i) => {
                        ...
                        const status = attendance[d.toDateString()]?.[selectedStudentId!];
                        const height = status === 'present' ? 100 : status === 'late' ? 60 : status === 'absent' ? 20 : 5;
                        const color = status === 'present' ? 'bg-emerald-500' : status === 'late' ? 'bg-amber-500' : status === 'absent' ? 'bg-red-500' : 'bg-slate-200';
                        
                        return (
                          <div key={i} className="flex flex-col items-center h-full justify-end w-1 gap-1">
                            <motion.div 
                              initial={{ height: 0 }}
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                         <span className="text-[8px] font-black text-slate-300 uppercase tracking-[0.2em] -rotate-6 opacity-30">Active Density</span>
                      </div>
                   </div>
                </div>
                ...
                {/* Recent Activity */}
                <div>
                   <h4 className="text-[9px] font-black tracking-[0.15em] text-slate-400 uppercase mb-2">Recent Record log</h4>
                   <div className="space-y-1.5">
                      {daysInView.slice(0, 4).reverse().map((date, i) => {
                        const status = attendance[date.toDateString()]?.[selectedStudentId!] || 'No Data';
                        return (
                          <div key={i} className="flex items-center justify-between p-2.5 bg-white border border-slate-100 rounded-lg">
                             <span className="text-[9px] font-bold text-slate-500">{date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }).toUpperCase()}</span>
                             <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded ${
                               status === 'present' ? 'bg-emerald-50 text-emerald-600' :
                               status === 'absent' ? 'bg-red-50 text-red-600' :
                               status === 'late' ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-slate-300'
                             }`}>
                                {status}
                             </span>
                          </div>
